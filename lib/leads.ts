...
export type BookingSubmission = {
  ...
  company?: string
  cargo: string
  /** Readable destination country (already resolved from the picker). */
  destination: string
  city?: string
  /** Readable freight mode, e.g. "Sea", "Air", "Express". */
  mode: string
  /** Cargo-ready date as an ISO yyyy-mm-dd string. */
  ready?: string
  /** Weight / volume, e.g. "12 CBM / 3,000 kg". */
  dims?: string
  details?: string
  /** "en" | "ar" — where the request came from. */
  locale?: string
}

/**
 * Persist a China→Middle East booking request as a Lead so it lands in /admin
 * next to chat, contact and RFQ leads. Freight mode and cargo-ready date are
 * folded into the readable summary message (no dedicated Lead columns needed),
 * so the team gets a complete booking brief.
 */
export async function createBookingLead(input: BookingSubmission) {
  const region = [input.destination, input.city].map((s) => s?.trim()).filter(Boolean).join(", ")
  const lines = [
    `Cargo: ${input.cargo}`,
    `Freight mode: ${input.mode}`,
    region && `Destination: ${region}`,
    input.ready && `Cargo ready: ${input.ready}`,
    input.dims && `Weight / volume: ${input.dims}`,
    input.company && `Company: ${input.company}`,
    input.phone && `Phone / WhatsApp: ${input.phone}`,
    input.locale === "ar" && `Submitted via: Arabic page`,
    input.details && `\nNotes:\n${input.details}`,
  ].filter(Boolean)

  return prisma.conversation.create({
    data: {
      title: `Booking — ${input.cargo} → ${input.destination}`.slice(0, 120),
      stage: "booking",
      messages: {
        create: { role: "user", content: `China → Middle East booking request\n\n${lines.join("\n")}` },
      },
      lead: {
        create: {
          ...
          email: input.email,
          phone: input.phone || null,
          productInterest: input.cargo,
          orderVolume: input.dims || null,
          preferredRegion: region || input.destination,
          sourcingTimeline: input.ready ? `Cargo ready ${input.ready}` : null,
          preferredContact: input.phone ? "Phone / WhatsApp" : "Email",
          notes: [`Freight mode: ${input.mode}`, input.details].filter(Boolean).join("\n\n") || null,
        },
      },
    },
    select: { id: true },
