...
/**
 * Emails the logistics team a complete China→Middle East booking brief.
 * No-ops gracefully when SENDER/RECIPIENT env vars are missing (the request is
 * still stored as a Lead, so nothing is lost).
 */
export async function sendBookingEmail(b: BookingEmailData): Promise<void> {
  try {
    const to = recipients();
    if (!process.env.SENDER_EMAIL || to.length === 0) {
      console.warn('[email] SENDER_EMAIL / RECIPIENT_EMAILS not set — skipping booking email');
      return;
    }
    ...
    const row = (label: string, value?: string | null) =>
      value
        ? `<tr><td style="padding:4px 12px 4px 0;color:#6b7280;">${label}</td><td style="padding:4px 0;color:#111;font-weight:600;">${esc(value)}</td></tr>`
        : '';
    const region = [b.destination, b.city].filter(Boolean).join(', ');

    const { error } = await resend.emails.send({
      from: process.env.SENDER_EMAIL,
      to,
      subject: `New cargo booking — ${b.cargo} → ${b.destination} (${b.mode})`,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:24px;background:#faf7f3;">
          <h1 style="color:#cc3433;margin:0 0 4px;">New Cargo Booking Request</h1>
          <p style="color:#6b7280;margin:0 0 20px;">China → Middle East${b.locale === 'ar' ? ' · submitted via Arabic page' : ''}.</p>
          <div style="background:#fff;border:1px solid #e7ddd1;border-radius:12px;padding:20px;">
            <table style="border-collapse:collapse;font-size:14px;">
              ${row('Name', b.name)}
              ${row('Email', b.email)}
              ...
              ${row('Cargo', b.cargo)}
              ${row('Freight mode', b.mode)}
              ${row('Destination', region)}
              ${row('Cargo ready', b.ready)}
              ${row('Weight / volume', b.dims)}
              ${b.details ? `<tr><td style="padding:12px 12px 4px 0;color:#6b7280;vertical-align:top;">Notes</td><td style="padding:12px 0 4px;color:#333;">${esc(b.details).replace(/\n/g, '<br>')}</td></tr>` : ''}
            </table>
          </div>
        </div>`,
    });
    if (error) console.error('Resend rejected the booking email:', error);
  } catch (error) {
